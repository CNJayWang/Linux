...
```
###函数
函数是让代码模块化，结构更加清晰，提高代码利用率，和其他编程一样函数需先定义再使用
...
```
函数返回值，可以显示增加`return`语句，如果不加将会以最后一条语句道执行结果为返回值

Shell函数的返回只能是整数，一般用来表示函数执行成功与否，`0`表示成功，其他值表示失败。如果`return`其他的数据，比如一个字符串，就会得到错误提示：`"numeric argument required"`
